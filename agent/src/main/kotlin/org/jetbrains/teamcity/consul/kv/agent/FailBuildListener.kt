...
import org.jetbrains.teamcity.consul.kv.isUrlParameter
...
class FailBuildListener : AgentLifeCycleAdapter() {
    override fun buildStarted(runningBuild: AgentRunningBuild) {
        ...
        if (parameters.asSequence().filter { isUrlParameter(it.key) && !it.value.isNullOrBlank() }.any()) {
            runningBuild.stopBuild("HashiCorp Consul Kv is not supported on this agent. Please add agent requirement for '${ConsulKvConstants.FEATURE_SUPPORTED_AGENT_PARAMETER}' parameter or run agent using Java 1.8")
        }
    }
}