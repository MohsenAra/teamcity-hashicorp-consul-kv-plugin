...
object ConsulKvReferencesUtil {
    ...
    fun hasReferences(parameters: Map<String, String>, namespaces: Collection<String>): Boolean {
        for ((_, value) in parameters) {
            ...
            val refs = getConsulKvReferences(value, namespaces)
            if (refs.isNotEmpty()) {
                return true
            }
        }
    ...
    fun getPath(ref: String, namespace: String): String {
        val prefix = ConsulKvConstants.CONSUL_KV_PARAMETER_PREFIX + if (isDefault(namespace)) "" else "$namespace:"
    ...
    fun getNamespace(ref: String): String {
        val value = ref.removePrefix(ConsulKvConstants.CONSUL_KV_PARAMETER_PREFIX)
    ...
    private fun getConsulKvReferences(value: String, namespaces: Collection<String>): Collection<String> {
        val prefixes = namespaces.map { ConsulKvConstants.CONSUL_KV_PARAMETER_PREFIX + if (isDefault(it)) "" else "$it:" }
