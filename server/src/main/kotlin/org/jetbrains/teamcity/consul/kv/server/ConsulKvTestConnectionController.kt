...
class ConsulKvTestConnectionController(server: SBuildServer, wcm: WebControllerManager,
                                         private val trustStoreProvider: SSLTrustStoreProvider                                        ) : BaseFormXmlController(server) {
    init {
        wcm.registerController("/admin/hashicorp-consul-kv-test-connection.html", this)
    }
    ...
    private fun doTestConnection(properties: Map<String, String>, xmlResponse: Element) {
        val processor = ConsulKvProjectConnectionProvider.getParametersProcessor()
        ...
        try {
            val settings = ConsulKvFeatureSettings(properties)
